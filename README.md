...
- **Activities** — optional point-earners any eligible kid can log (e.g. Exercise). Not required, no streaks, nothing "missed". Two modes: **every day** (each kid can log once per day) or **once each** (each kid can earn it a single time). Kid taps "Log it!" → pending → approve → points. Restrict to specific kids or leave open to all.
