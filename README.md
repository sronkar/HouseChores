...
- **Streak award** (optional, per recurring chore) — a bonus credited on top of the chore's points each time the streak is kept. The parent sets a base award; it grows **+1 point for every full 10 days** of streak (10-day streak → base+1, 20 → base+2, …). Shown as 🎁 on the kid's row and in the approval queue; folded into that completion's `earn_event`.
