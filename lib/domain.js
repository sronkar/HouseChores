...
export function addActivity({ name, emoji = "⭐", points = 5, kidIds = [], mode = "daily" }) {
  ...
  return db()
    ...
    .run(name, emoji, points, ids.join(","), mode === "once" ? "once" : "daily").lastInsertRowid;
}
export function updateActivity(id, { name, emoji, points, kidIds, mode }) {
  ...
  db()
    ...
    .run(name, emoji, points, ids.join(","), mode === "once" ? "once" : "daily", id);
}
...
// The kid's most recent log for an activity in the relevant window.
  ...
  return listActivities()
    ...
    .map((a) => {
      ...
      return {
        activity: a,
        task,
        canLog: !task, // no pending/approved log in the window
        status: task?.status ?? null,
      };
    });
}

// Kid logs an activity → pending, awaiting approval. Deduped per window.
export function logActivity(activityId, kidId) {
  ...
  if (activityTaskFor(activityId, kidId, act.mode, today)) return false; // already logged
  d.prepare(
