...
export function addTemplate({ name, emoji = "✅", points = 5, kidIds = [], streakAward = 0 }) {
  const d = db();
  const id = d
    .prepare("INSERT INTO chore_template(name,emoji,points,streak_award) VALUES(?,?,?,?)")
    .run(name, emoji, points, Math.max(0, Math.round(streakAward) || 0)).lastInsertRowid;
  const ins = d.prepare("INSERT INTO chore_template_kid(template_id,kid_id) VALUES(?,?)");
  for (const kid of kidIds) ins.run(id, kid);
  return id;
}
export function updateTemplate(id, { name, emoji, points, kidIds, streakAward = 0 }) {
  const d = db();
  d.prepare("UPDATE chore_template SET name=?, emoji=?, points=?, streak_award=? WHERE id=?").run(
    name, emoji, points, Math.max(0, Math.round(streakAward) || 0), id
  );
...
// Streak bonus for a chore: base award, +1 for every full 10 days of streak.
export function streakAwardFor(baseAward, streak) {
  return baseAward > 0 ? baseAward + Math.floor(streak / 10) : 0;
}
...
// { ownerKidId, periodKey } for a given date, per cadence.
function altOwnerForDate(alt, date = todayStr()) {
  const kids = altRotation(alt);
  if (kids.length === 0) return null;
  const N = kids.length;
  if (alt.cadence === "on_completion") {
    const idx = ((alt.turn_index % N) + N) % N;
    return { ownerKidId: kids[idx], periodKey: date };
  }
  if (alt.cadence === "weekly") {
    const wk = Math.floor(daysBetween(alt.anchor_date, date) / 7);
    const idx = ((wk % N) + N) % N;
    return { ownerKidId: kids[idx], periodKey: addDays(alt.anchor_date, wk * 7) };
  }
  // daily
  const day = daysBetween(alt.anchor_date, date);
  const idx = ((day % N) + N) % N;
  return { ownerKidId: kids[idx], periodKey: date };
}
...
export function listAltChores() {
  ...
  for (const a of alts) a.kidIds = parseKidIds(a.kid_ids);
  return alts;
}
export function addAltChore({ name, emoji = "🔁", points = 5, kidIds = [], cadence = "daily" }) {
  const ids = kidIds.filter((n) => Number.isInteger(n) && n > 0);
  return db()
    .prepare(
      `INSERT INTO alt_chore(name,emoji,points,kid_ids,cadence,anchor_date)
       VALUES(?,?,?,?,?,?)`
    )
    .run(name, emoji, points, ids.join(","), cadence, todayStr()).lastInsertRowid;
}
export function updateAltChore(id, { name, emoji, points, kidIds, cadence }) {
  const ids = (kidIds || []).filter((n) => Number.isInteger(n) && n > 0);
  db()
    .prepare("UPDATE alt_chore SET name=?, emoji=?, points=?, kid_ids=?, cadence=? WHERE id=?")
    .run(name, emoji, points, ids.join(","), cadence, id);
}
  ...
  for (const alt of alts) {
    ...
    const exists =
      alt.cadence === "on_completion"
        ? d.prepare("SELECT 1 FROM task WHERE alt_id=? AND status IN ('open','pending')").get(alt.id)
        : d.prepare("SELECT 1 FROM task WHERE alt_id=? AND date=?").get(alt.id, owner.periodKey);
    if (!exists) {
...
export function cloneTemplate(id) {
  ...
  return addTemplate({ name: t.name + " (copy)", emoji: t.emoji, points: t.points, kidIds, streakAward: t.streak_award });
}
...
export function getKidDay(kidId, date = todayStr()) {
  ...
  const assigned = d
    .prepare(
      `SELECT t.*, ct.streak_award FROM task t
  ...
  for (const t of assigned) {
    ...
    t.bonus = streakAwardFor(t.streak_award || 0, projected);
  }
...
export function pendingQueue() {
  const rows = db()
    .prepare(
      `SELECT t.*, ct.streak_award, k.name AS kid_name, k.emoji AS kid_emoji, k.color AS kid_color
       FROM task t
       JOIN kid k ON k.id = COALESCE(t.done_by_kid_id, t.assigned_kid_id)
       LEFT JOIN chore_template ct ON ct.id=t.template_id
  ...
  for (const t of rows) {
    ...
    t.bonus =
      t.source === "recurring" && t.template_id && t.streak_award > 0
        ? streakAwardFor(t.streak_award, choreStreak(t.template_id, t.done_by_kid_id ?? t.assigned_kid_id, t.date))
        : 0;
  }
  return rows;
}
...
export function approveTask(taskId) {
  ...
  if (t.source === "recurring" && t.template_id) {
    const tpl = d.prepare("SELECT streak_award FROM chore_template WHERE id=?").get(t.template_id);
    if (tpl && tpl.streak_award > 0) {
      ...
      award += streakAwardFor(tpl.streak_award, streak);
    }
  }
