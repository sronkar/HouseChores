...
// Debounce: nudge only when there's something new, and not more than once per window.
export function nudgeDue(count = pendingDigest().count, now = Date.now()) {
  if (count <= 0) return false;
  const lastCount = Number(getSetting("nudge_last_count") || 0);
  const lastAt = Number(getSetting("nudge_last_at") || 0);
  const { minMinutes } = getNudgeConfig();
  if (count > lastCount) return true; // new pending appeared
...
// If count dropped to 0 (all approved), reset the debounce baseline.
export function syncNudgeBaseline() {
  const c = pendingDigest().count;
  if (c === 0) setSetting("nudge_last_count", "0");
}
