...
function open() {
  // DATA_DIR lets the SQLite file live on a mounted volume in production so it
  // survives redeploys; defaults to ./data for local dev.
...
function migrate(db) {
  ...
  if (!taskCols.includes("board_template_id")) {
    db.exec("ALTER TABLE task ADD COLUMN board_template_id INTEGER");
  }
  if (!taskCols.includes("activity_id")) {
    db.exec("ALTER TABLE task ADD COLUMN activity_id INTEGER");
  }

  // Optional point-earning activities (e.g. Exercise). Any eligible kid can log
  // them for points; no streaks. mode 'daily' = once/kid/day, 'once' = once/kid ever.
  db.exec(`
    CREATE TABLE IF NOT EXISTS activity (
      ...
      emoji TEXT NOT NULL DEFAULT '⭐',
      points INTEGER NOT NULL DEFAULT 5,
      kid_ids TEXT NOT NULL DEFAULT '',   -- CSV of eligible kid ids; '' = all kids
      mode TEXT NOT NULL DEFAULT 'daily', -- daily | once
      active INTEGER NOT NULL DEFAULT 1,
      created_at TEXT NOT NULL DEFAULT (datetime('now'))
    );
  `);

  // Recurring board tasks: an unassigned bounty that appears on chosen weekdays.
  db.exec(`
    CREATE TABLE IF NOT EXISTS board_template (
      ...
      emoji TEXT NOT NULL DEFAULT '📌',
      points INTEGER NOT NULL DEFAULT 5,
      weekdays TEXT NOT NULL DEFAULT '',   -- CSV of 0-6 (Sun=0); '' = every day
      active INTEGER NOT NULL DEFAULT 1,
      created_at TEXT NOT NULL DEFAULT (datetime('now'))
    );
  `);
  ...
  `);
  const altCols = db.prepare("PRAGMA table_info(alt_chore)").all().map((c) => c.name);
  if (!altCols.includes("turn_offset")) {
    // Phase shift so same-day chores can land on different kids.
    db.exec("ALTER TABLE alt_chore ADD COLUMN turn_offset INTEGER NOT NULL DEFAULT 0");
  }
